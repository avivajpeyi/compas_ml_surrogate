...
from . import ClassCOMPAS, ClassMSSFR, selection_effects
...
class CosmicIntegrator(object):
    ...
    def __init__(
        ...
        if Cosmology == "Custom Flat":
            self.cosmology = FlatLambdaCDM(
                H0=hubbleConstant * u.km / u.s / u.Mpc, Om0=omegaMatter
            )
        ...
        if self.verbose:
            print(
                "Creating instance COMPAS class User has to still set DCO and Data"
            )
        # setting Mlower/Mupper etc to None to force warning for user
        self.COMPAS = ClassCOMPAS.COMPASData(
            path=self.pathCOMPAS,
            lazyData=True,
            Mlower=None,
            Mupper=None,
            binaryFraction=None,
        ...
        self.MSSFR = ClassMSSFR.MSSFR(
            metallicityGrid=None, cosmo=self.cosmology
        )
    ...
    def createConcentricRedshiftShells(self):
        ...
        redshiftEdges = np.linspace(
            self.minRedshift, self.maxRedshift, self.nrRedshiftBins + 1
        )  # The bin edges in redshift
        # Central value of each redshift bin, this is the value used in cosmic Int
        self.Shell_dz = np.diff(redshiftEdges)
        self.Shell_centerRedshift = 0.5 * (
            redshiftEdges[:-1] + redshiftEdges[1:]
        )
        # Corresponding luminosity Distances [Mpc]
        self.Shell_luminosityDistance = [
            x.value
            for x in self.cosmology.luminosity_distance(
                self.Shell_centerRedshift
            )
        ]
        ...
        comovEdges = [
            x.value * 1e-9
            for x in self.cosmology.comoving_volume(redshiftEdges)
        ]
    ...
    def setAgeBirthSystems(self):
        ...
        self.PerSystemPerRedshift_ageBirth = np.zeros(
            shape=(int(self.nrRedshiftBins), len(self.COMPAS.delayTimes))
        )
        ...
        for nrz, redshift in enumerate(self.Shell_centerRedshift):
            ...
            delayTimeGyr = self.COMPAS.delayTimes / 1000.0
    ...
    def setRedshiftBirthSystems(self):
        self.PerSystemPerRedshift_redshiftBirth = np.zeros(
            shape=(int(self.nrRedshiftBins), len(self.COMPAS.delayTimes))
        )
        ...
        for nrR, row in enumerate(self.PerSystemPerRedshift_ageBirth):
            ...
            if self.RedshiftTabulated:
                ...
                inds = np.digitize(
                    row[maskPhysical], self.redshiftAgeTable[:, 1]
                )
                ...
                for nra, age in enumerate(maskPhysical):
                    redshifts[nra] = newton(
                        lambda x: self.cosmology.age(x).value - age, 0
                    )
            # fill in values
            self.PerSystemPerRedshift_redshiftBirth[nrR][maskUnphysical] = -1
            self.PerSystemPerRedshift_redshiftBirth[nrR][
                maskPhysical
            ] = redshifts
    ...
    def setBirthTimesAnd2Darrays(self):
        ...
        if self.COMPAS.delayTimes is not None:
            ...
            # dN Gpc-3 per year at z=redshift.     #each row is redshift merger
            self.PerSystemPerRedshift_ratesIntrinsic = np.zeros(
                shape=(int(self.nrRedshiftBins), len(self.COMPAS.delayTimes))
            )
            # dN per year in detector from shell
            self.PerSystemPerRedshift_ratesObserved = np.zeros(
                shape=(int(self.nrRedshiftBins), len(self.COMPAS.delayTimes))
            )
        else:
            print()
    ...
    def cosmologicalIntegration(self):
        ...
        for nr in range(int(self.nrRedshiftBins)):
            for nrZ, Z in enumerate(self.COMPAS.metallicityGrid):
                ...
                MSSFR = self.MSSFR.returnMSSFR(
                    metallicity=Z,
                    agesBirth=self.PerSystemPerRedshift_ageBirth[nr][maskZ],
                    redshiftBirth=self.PerSystemPerRedshift_redshiftBirth[nr][
                        maskZ
                    ],
                )
                RatesZ = np.divide(
                    MSSFR, self.COMPAS.totalMassEvolvedPerZ[nrZ]
                )
                self.PerSystemPerRedshift_ratesIntrinsic[nr][
                    maskZ
                ] = RatesZ  # intrinric dN Gpc-3 per year at z=redshift.
                probObservingZ = selection_effects.detection_probability(
                    self.COMPAS.mass1[maskZ],
                    self.COMPAS.mass2[maskZ],
                    self.Shell_centerRedshift[nr],
                    self.Shell_luminosityDistance[nr],
                    self.GWdetector_snrThreshold,
                    sensitivity=self.GWdetector_sensitivity,
                )
                NrMergersInShell = np.multiply(
                    RatesZ, self.Shell_volume[nr]
                )  # intrinsic rate per system per shell
                NrMergersInShell = NrMergersInShell * (
                    1.0 / (1.0 + self.Shell_centerRedshift[nr])
                )  # rate observer frame per system per shell
                self.PerSystemPerRedshift_ratesObserved[nr][
                    maskZ
                ] = np.multiply(
                    NrMergersInShell, probObservingZ
                )  # observed  dN per year prob between 0-1
