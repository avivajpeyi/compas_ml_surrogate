...
class MSSFR(object):
    ...
    def __init__(
        ...
        self.solarMetallicity = (
            solarMetallicity  # fraction mass in metals solar
        )
        ...
        self.customSFR = [
            ...
            None,
            None,
        ]  # [a,b,c,d] see function customSFR
        ...
        if cosmo == "oldDefault":
            self.cosmology = FlatLambdaCDM(
                H0=67.8 * u.km / u.s / u.Mpc, Om0=0.308
            )
    ...
    def calculateMetallicityBinEdges(self):
        ...
        if self.binInLogSpace:
            ...
            b = (
                logMetallicities[:-1]
                + (logMetallicities[1:] - logMetallicities[:-1]) / 2.0
            )
            b = (
                10.0**b
            )  # the boundaries for integration are not in log space so
            # convert to "normal" numbers.
        else:
            b = (
                self.metallicityGrid[1:] - self.metallicityGrid[:-1]
            ) / 2.0 + self.metallicityGrid[:-1]
    ...
    def returnFractionMZ_GSMF(self, Zlower, Zupper, redshift):
        ...
        if self.ZMprescription == "Ma et al. (2016)":
            ...
            print(
                "Danger Savaglio2005Bisector not fully working turnover in function"
            )
            Mupper = 10 ** self.Savaglio2005ZM(Zupper_Zsun, z)
            Mlower = 10 ** self.Savaglio2005ZM(Zlower_Zsun, z)
        else:
            raise ValueError(
                "The mass-metallicity prescription is not recognised.\n"
        ...
        if (
            self.GSMFprescription == "Panter et al. (2004) Single"
            ...
            logMc, phi1, a1, phi2, a2 = self.lineairFitDoubleSchechterFurlong(
                z
            )
        else:
            raise ValueError(
                "This GSMF prescription is not recognised.\n"
                + "Current options are:\n"
        ...
        if self.GSMFprescription in [
            "Panter et al. (2004) Single",
            "Furlong et al. (2015) Single",
        ]:
            schechterUpperIncomplete = scipy.special.gammainc(
                alpha + 2, Mupper / Mc
            )
            schechterLowerIncomplete = scipy.special.gammainc(
                alpha + 2, Mlower / Mc
            )
            ...
            schechter1Incomplete = (
                scipy.special.gammainc(a1 + 2.0, Mupper / (Mc))
                * schechter1Complete
            )
            schechter2Complete = scipy.special.gamma(a2 + 2.0)
            schechter2Incomplete = (
                scipy.special.gammainc(a2 + 2.0, Mupper / (Mc))
                * schechter2Complete
            )
            integralUpperBound = (
                phi1 * schechter1Incomplete + phi2 * schechter2Incomplete
            ) / (phi1 * schechter1Complete + phi2 * schechter2Complete)
            # lower bound
            schechter1Complete = scipy.special.gamma(a1 + 2.0)
            schechter1Incomplete = (
                scipy.special.gammainc(a1 + 2.0, Mlower / (Mc))
                * schechter1Complete
            )
            schechter2Complete = scipy.special.gamma(a2 + 2.0)
            schechter2Incomplete = (
                scipy.special.gammainc(a2 + 2.0, Mlower / (Mc))
                * schechter2Complete
            )
            integralLowerBound = (
                phi1 * schechter1Incomplete + phi2 * schechter2Incomplete
        ...
        for nrv, values in enumerate(fitvalues):
            ...
            for nrz, redshift in enumerate(thresholds[:-1]):
                ...
                if nrz == 0:
                    ...
                    dz = 0.5 - z[mask]
                    interpolatedValue = values[nrz + 1] - np.multiply(
                        dz, dydz[nrz]
                    )
                else:
                    # interpolate up
                    dz = z[mask] - redshift
                    interpolatedValue = values[nrz] + np.multiply(
                        dz, dydz[nrz]
                    )
    ...
    def lineairFitDoubleSchechterFurlong(self, z):
        ...
        ratioPhi1Phi2 = np.divide(
            fitphi2, fitphi1
        )  # Do it in this order because phi2 is zero
    ...
    def Savaglio2005ZM(self, ZZsun, z):
        ...
        logM[mask2] = ((-b) + np.sqrt(d[mask2])) / (
            2 * a
        )  # The ones with a non-negative root
    ...
    def returnSFR(self, redshifts, age):
        if self.SFRprescription == "Madau et al. (2014)":
            ...
            raise ValueError(
                "This SFR prescription is not recognised.\n"
                + "Current options are:\n"
    ...
    def SFR_Madau(self, z):
        ...
        """
        SFR = (
            0.015 * ((1 + z) ** 2.7) / (1 + ((1 + z) / 2.9) ** 5.6) * 1e9
        )  # [1e9 for GPc-3]
    ...
    def SFR_Madau2(self, z):
        ...
        """
        SFR = (
            0.01 * ((1 + z) ** 2.6) / (1 + ((1 + z) / 3.2) ** 6.2) * 1e9
        )  # [1e9 for GPc-3]
        return SFR  # [Msun yr-1 Gpc-3] in comoving volume
    ...
    def SFR_Strolger(self, tGyrs):
        ...
        SFR = (
            ...
            * a
            * (
                tGyrs**b * np.exp(-tGyrs / c)
                + d * np.exp(d * (tGyrs - t0) / c)
            )
        )
        if np.isnan(SFR.any()):
            raise ValueError(
                "Nan in SFR calculation for %s" % (self.SFRprescription)
            )
        return SFR  # Msun yr-1 Gpc-3 in comoving volume
    ...
    def SFR_Neijssel(self, z):
        ...
        """
        SFR = (
            0.01 * ((1 + z) ** 2.77) / (1 + ((1 + z) / 2.9) ** 4.7) * 1e9
        )  # [1e9 for GPc-3]
        return SFR  # [Msun yr-1 Gpc-3] in comoving volume
    ...
    def returnFractionLogNormal(self, Zlower, Zupper, redshift):
        ...
        if self.logNormalPrescription == "Neijssel Phenomenological":
            ...
            if None in self.customLogNormal:
                raise ValueError(
                    "customLogNormal not set .customLogNormal =[Z0, alpha, sigma]"
                )
            ...
            raise ValueError(
                "This logNormal prescription is not recognised.\n"
                + "Current options are:\n"
                + "Phenomenological, Custom Phenomenological' \n"
                + "Apologies but I will now break"
            )
        Zmean = Z0 * (10 ** (alpha * redshift))
        mu = np.log(Zmean) - (sigma * sigma) / 2.0
    ...
    def returnMSSFR(
        self, metallicity=None, agesBirth=None, redshiftBirth=None
    ):
        """
        ...
        if self.Zprescription == "MZ_GSMF":
            fractions[mask] = self.returnFractionMZ_GSMF(
                Zlower, Zupper, redshiftBirth[mask]
            )
        elif self.Zprescription == "logNormal":
            fractions[mask] = self.returnFractionLogNormal(
                Zlower, Zupper, redshiftBirth[mask]
            )
        else:
            raise ValueError(
                "self.Zprescription not recognized, MZ_GSMF or logNormal?"
            )
        # Always raise an error if the fraction is unphysical
        if ((fractions > 1).any()) or ((fractions < 0).any()):
            raise ValueError(
                "unphysical SFR fraction check metallicity binning limits"
            )
