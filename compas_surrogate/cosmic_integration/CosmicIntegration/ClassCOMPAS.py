...
class COMPASData(object):
    def __init__(
        ...
        if self.path is None:
            ...
            raise ValueError(
                "h5 file not found. Wrong path given? {}".format(path)
            )
    ...
    def setCOMPASDCOmask(
        ...
        self.CHE_mask = (
            np.in1d(dco_seeds, che_seeds)
            if types == "CHE_BBH" or types == "NON_CHE_BBH"
            else np.repeat(False, len(dco_seeds))
        )

        # if user wants to mask on Hubble time use the flag, otherwise just set all to True
        hubble_mask = (
            hubble_flag.astype(bool)
            if withinHubbleTime
            else np.repeat(True, len(dco_seeds))
        )
        ...
        type_masks["CHE_BBH"] = (
            np.logical_and(self.CHE_mask, type_masks["BBH"])
            if types == "CHE_BBH"
            else np.repeat(False, len(dco_seeds))
        )
        type_masks["NON_CHE_BBH"] = (
            np.logical_and(np.logical_not(self.CHE_mask), type_masks["BBH"])
        ...
        if noRLOFafterCEE or pessimistic:
            ...
            ce_seeds = self.get_COMPAS_variables(
            ...
            if noRLOFafterCEE:
                rlof_flag = self.get_COMPAS_variables(
            ...
            if pessimistic:
                pessimistic_flag = self.get_COMPAS_variables(
                    "BSE_Common_Envelopes", "Optimistic_CE"
                )[dco_from_ce].astype(bool)
                pessimistic_seeds = np.unique(dco_ce_seeds[pessimistic_flag])
                pessimistic_mask = np.logical_not(
                    np.in1d(dco_seeds, pessimistic_seeds)
                )
            else:
        ...
        self.DCOmask = (
            type_masks[types] * hubble_mask * rlof_mask * pessimistic_mask
        )
        self.BBHmask = (
            type_masks["BBH"] * hubble_mask * rlof_mask * pessimistic_mask
        )
        self.BHNSmask = (
            type_masks["BHNS"] * hubble_mask * rlof_mask * pessimistic_mask
        )
        self.DNSmask = (
            type_masks["BNS"] * hubble_mask * rlof_mask * pessimistic_mask
        )
        self.CHE_BBHmask = (
            type_masks["CHE_BBH"] * hubble_mask * rlof_mask * pessimistic_mask
        )
        self.NonCHE_BBHmask = (
            type_masks["NON_CHE_BBH"]
            * hubble_mask
        ...
        self.allTypesMask = (
            type_masks["all"] * hubble_mask * rlof_mask * pessimistic_mask
        )
    ...
    def setGridAndMassEvolved(self):
        ...
        if self.initialZ is None:
            self.initialZ = Data["BSE_System_Parameters"][
                "Metallicity@ZAMS(1)"
            ][()]
    ...
    def setCOMPASData(self):
        ...
        initial_seeds, initial_Z = self.get_COMPAS_variables(
        ...
        self.delayTimes = np.add(
            formation_times[self.DCOmask], coalescence_times[self.DCOmask]
        )
        ...
        if self.lazyData:
            ...
            self.Hubble = self.get_COMPAS_variables(
    ...
    def get_COMPAS_variables(self, hdf5_file, var_names):
        ...
        with h5.File(self.path, "r") as compas_file:
            ...
            # else return each variable in a list
                ...
                return [
                    compas_file[hdf5_file][var_name][...].squeeze()
    ...
    def set_sw_weights(self, column_name):
        ...
        if column_name is not None:
            self.sw_weights = self.get_COMPAS_variables(
                "BSE_Double_Compact_Objects", column_name
            )[self.DCOmask]

    def find_star_forming_mass_per_binary_sampling(
        self,
        m1=0.01,
        m2=0.08,
        m3=0.5,
        ...
        a23=1.3,
        a34=2.3,
        primary_mass_inverse_CDF=None,
        mass_ratio_inverse_CDF=None,
        SAMPLES=20000000,
    ):
        """
        ...
        if primary_mass_inverse_CDF is None:
            primary_mass_inverse_CDF = lambda U: inverse_CDF_IMF(
                U, m1=m1, m2=m2, m3=m3, m4=m4, a12=a12, a23=a23, a34=a34
            )
        ...
        primary_mass = (
            primary_mass_inverse_CDF(np.random.rand(SAMPLES)) * u.Msun
        )
        ...
        secondary_mass[binary_mask] = (
            primary_mass[binary_mask] * mass_ratio[binary_mask]
        )
        ...
        primary_mask = np.logical_and(
            primary_mass >= self.Mlower, primary_mass <= self.Mupper
        )
        ...
        total_mass_COMPAS = np.sum(primary_mass[full_mask]) + np.sum(
            secondary_mass[full_mask]
        )
...
def IMF(m, m1=0.01, m2=0.08, m3=0.5, m4=200.0, a12=0.3, a23=1.3, a34=2.3):
    ...
    if isinstance(m, float):
        ...
        imf_vals[np.logical_and(m >= m1, m < m2)] = b1 * m[
            np.logical_and(m >= m1, m < m2)
        ] ** (-a12)
        imf_vals[np.logical_and(m >= m2, m < m3)] = b2 * m[
            np.logical_and(m >= m2, m < m3)
        ] ** (-a23)
        imf_vals[np.logical_and(m >= m3, m < m4)] = b3 * m[
            np.logical_and(m >= m3, m < m4)
        ] ** (-a34)
        return imf_vals
...
def CDF_IMF(m, m1=0.01, m2=0.08, m3=0.5, m4=200.0, a12=0.3, a23=1.3, a34=2.3):
    ...
    if isinstance(m, float):
        if m <= m1:
            ...
            return CDF_IMF(m2) + b2 / (1 - a23) * (
                m ** (1 - a23) - m2 ** (1 - a23)
            )
        elif m <= m4:
            return CDF_IMF(m3) + b3 / (1 - a34) * (
                m ** (1 - a34) - m3 ** (1 - a34)
            )
        else:
            return 0
        ...
        CDF = np.zeros(len(m))
        CDF[np.logical_and(m >= m1, m < m2)] = (
            b1
            / (1 - a12)
            * (
                m[np.logical_and(m >= m1, m < m2)] ** (1 - a12)
                - m1 ** (1 - a12)
            )
        )
        CDF[np.logical_and(m >= m2, m < m3)] = CDF_IMF(m2) + b2 / (1 - a23) * (
            m[np.logical_and(m >= m2, m < m3)] ** (1 - a23) - m2 ** (1 - a23)
        )
        CDF[np.logical_and(m >= m3, m < m4)] = CDF_IMF(m3) + b3 / (1 - a34) * (
            m[np.logical_and(m >= m3, m < m4)] ** (1 - a34) - m3 ** (1 - a34)
        )
...
def inverse_CDF_IMF(
    U, m1=0.01, m2=0.08, m3=0.5, m4=200, a12=0.3, a23=1.3, a34=2.3
):
    """
    ...
    masses[np.logical_and(U > F1, U <= F2)] = np.power(
        (1 - a12) / b1 * (U[np.logical_and(U > F1, U <= F2)] - F1)
        + m1 ** (1 - a12),
        1 / (1 - a12),
    )
    masses[np.logical_and(U > F2, U <= F3)] = np.power(
        (1 - a23) / b2 * (U[np.logical_and(U > F2, U <= F3)] - F2)
        + m2 ** (1 - a23),
        1 / (1 - a23),
    )
    masses[np.logical_and(U > F3, U <= F4)] = np.power(
        (1 - a34) / b3 * (U[np.logical_and(U > F3, U <= F4)] - F3)
        + m3 ** (1 - a34),
        1 / (1 - a34),
    )
    return masses
