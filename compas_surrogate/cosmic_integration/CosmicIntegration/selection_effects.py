...
def detection_probability(
    m1, m2, redshift, distance, snr_threshold, sensitivity="design"
):
    """
    ...
    return detection_probability_from_snr(
        snr_value=interpolated_snr, snr_threshold=snr_threshold
    )
...
class Interpolator:
    ...
    def _custom(self, m1, m2):
        ...
        snr_selection = self.snr_grid[
            [i - 1, i - 1, i, i], [j - 1, j, j - 1, j]
        ]
        ...
        return np.average(snr_selection, weights=1.0 / distance)
...
def detection_probability_from_snr(snr_value, snr_threshold, n_thetas=1e6):
    ...
    if _random_thetas is None or _random_thetas.size != n_thetas:
        ...
        Fps = 0.5 * np.cos(2 * zetas) * (1 + cos_thetas**2) * np.cos(
            2 * phis
        ) - np.sin(2 * zetas) * cos_thetas * np.sin(
            2 * phis
        )  # 3.29a
        Fxs = 0.5 * np.sin(2 * zetas) * (1 + cos_thetas**2) * np.cos(
            2 * phis
        ) + np.cos(2 * zetas) * cos_thetas * np.sin(
            2 * phis
        )  # 3.29b
        _random_thetas = np.sqrt(
            0.25 * Fps**2 * (1 + cos_incs**2) ** 2
            + Fxs**2 * cos_incs**2
        )  # 3.31
    ...
    theta_min = np.atleast_1d(theta_min).flatten()
    detection_prob = np.zeros_like(theta_min)
    detection_prob[theta_min <= 1] = 1.0 - (
        (np.digitize(theta_min[theta_min <= 1], _random_thetas) - 1.0)
        / float(n_thetas)
    )
