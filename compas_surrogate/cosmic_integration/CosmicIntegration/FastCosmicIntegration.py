...
from . import ClassCOMPAS, selection_effects
    ...
    for i in range(n_binaries):
        ...
        # calculate formation rate (see Neijssel+19 Section 4) - note this uses dPdlogZ for *closest* metallicity
            ...
            formation_rate[i, :] = (
                n_formed
                * dPdlogZ[
                    :, np.digitize(COMPAS_metallicites[i], metallicities)
                ]
                / p_draw_metallicity
                * COMPAS_weights[i]
            )
        ...
        if first_too_early_index > 0:
            ...
            z_of_formation_index = np.ceil(
                z_of_formation / redshift_step
            ).astype(int)
...
def compute_snr_and_detection_grids(
    ...
    M1_array = (
        Mt_array * 0.5 * (1.0 + np.sqrt(1.0 - 4 * eta_array[:, np.newaxis]))
    )
    ...
    detection_probability_from_snr = (
        selection_effects.detection_probability_from_snr(
            snr_array, snr_threshold
        )
    )
...
def find_detection_probability(
    ...
    for i in range(n_binaries):
        ...
        snrs[Mc_below_max] = snr_grid_at_1Mpc[
            eta_index, Mc_index[Mc_below_max]
        ]
        ...
        snr_below_max = detection_list_index < len(
            detection_probability_from_snr
        )
        ...
        detection_probability[
            i, snr_below_max
        ] = detection_probability_from_snr[detection_list_index[snr_below_max]]
...
def find_detection_rate(
    ...
    assert (
        max_redshift_detection <= max_redshift
    ), "Maximum detection redshift cannot be below maximum redshift"
    assert (
        m1_min <= m1_max
    ), "Minimum sampled primary mass cannot be above maximum sampled primary mass"
    assert np.logical_and(
        fbin >= 0.0, fbin <= 1.0
    ), "Binary fraction must be between 0 and 1"
    assert (
        Mc_step < Mc_max
    ), "Chirp mass step size must be less than maximum chirp mass"
    assert (
        eta_step < eta_max
    ), "Symmetric mass ratio step size must be less than maximum symmetric mass ratio"
    ...
    if redshift_step > max_redshift_detection:
        warnings.warn(
            "Redshift step is greater than maximum detection redshift",
            stacklevel=2,
        )
    if Mc_step > 1.0:
        warnings.warn(
            "Chirp mass step is greater than 1.0, large step sizes can produce unpredictable results",
            stacklevel=2,
        )
    if eta_step > 0.1:
        warnings.warn(
            "Symmetric mass ratio step is greater than 0.1, large step sizes can produce unpredictable results",
            stacklevel=2,
        )
    if snr_step > 1.0:
        warnings.warn(
            "SNR step is greater than 1.0, large step sizes can produce unpredictable results",
            stacklevel=2,
        )

    # start by getting the necessary data from the COMPAS file
    COMPAS = ClassCOMPAS.COMPASData(
        path,
        Mlower=m1_min,
        Mupper=m1_max,
        m2_min=m2_min,
    ...
    COMPAS.setCOMPASDCOmask(
        types=dco_type,
        withinHubbleTime=merges_hubble_time,
        pessimistic=pessimistic_CEE,
        noRLOFafterCEE=no_RLOF_after_CEE,
    )
    ...
    if use_sampled_mass_ranges:
        COMPAS.Mlower = (
            min(m1[m1 != m2]) * u.Msun
        )  # the m1!=m2 ensures we don't include masses set equal through RLOF at ZAMS
    ...
    chirp_masses = (COMPAS.mass1 * COMPAS.mass2) ** (3 / 5) / (
        COMPAS.mass1 + COMPAS.mass2
    ) ** (1 / 5)
    ...
    if max(chirp_masses) * (1 + max_redshift_detection) > Mc_max:
        warnings.warn(
            "Maximum chirp mass used for detectability calculation is below maximum binary chirp mass * (1+maximum redshift for detectability calculation)",
            stacklevel=2,
        )
    ...
    sfr = find_sfr(
        redshifts, a=aSF, b=bSF, c=cSF, d=dSF
    )  # functional form from Madau & Dickinson 2014

    # Calculate the representative SF mass
    Average_SF_mass_needed = COMPAS.mass_evolved_per_binary * COMPAS.n_systems
    print(
        "Average_SF_mass_needed = ", Average_SF_mass_needed
    )  # print this, because it might come in handy to know when writing up results :)
    n_formed = (
        sfr / Average_SF_mass_needed
    ...
    if np.log(np.min(COMPAS.initialZ)) != np.log(
        np.max(COMPAS.initialZ)
    ):  # Will perform integral over metallicities
        (
            dPdlogZ,
            metallicities,
            p_draw_metallicity,
    ...
    detection_rate = (
        merger_rate[:, :n_redshifts_detection]
        * detection_probability
    ...
    if (
        merger_output_filename != None
    ):  # Store merger rates in an output text file if specified
        with open(path + merger_output_filename, "w") as output:
            output.write(
                "Mass1atMerger \t Mass2atMerger \t MergerRedshift \t MergerRate \n"
            )
            ...
            for i in range(n_redshifts_detection):
                for j in range(n_binaries):
                    if merger_rate[j][i] > 0:
                        output.write(
                            f"{COMPAS.mass1[j]:.5f}\t{COMPAS.mass2[j]:.5f}\t{redshifts[i]:.5f}\t{merger_rate[j][i]:.10f}\n"
                        )
...
def append_rates(
    ...
    with h5.File(path, "r+") as h_new:
        ...
        new_rate_group = "Rates_mu0{}_muz{}_alpha{}_sigma0{}_sigmaz{}".format(
            mu0, muz, alpha, sigma0, sigmaz
        )
        ...
        if append_binned_by_z:
            ...
            redshift_bins = np.arange(
                0, redshifts[-1] + redshift_binsize, redshift_binsize
            )
            fine_binsize = np.diff(redshifts)[
                0
            ]  # Assunming your redshift bins are equally spaced!!
            ...
            crude_volumes = (
                cosmology.comoving_volume(redshift_bins).to(u.Gpc**3).value
            )
            ...
            # convert redshifts to volumnes and ensure all volumes are in Gpc^3
            fine_volumes = (
                cosmology.comoving_volume(redshifts).to(u.Gpc**3).value
            )
            fine_shell_volumes = np.diff(fine_volumes)
            fine_shell_volumes = np.append(
                fine_shell_volumes, fine_shell_volumes[-1]
            )
            ...
            binned_merger_rate = np.zeros(
                (N_dco, len(redshift_bins) - 1)
            )  # create an empty list to fill
            binned_detection_rate = np.zeros(
                (N_dco, len(redshift_bins) - 1)
            )  # create an empty list to fill
            ...
            for i in range(len(redshift_bins) - 1):
                ...
                binned_merger_rate[:, i] = (
                    np.sum(
                        N_dco_in_z_bin[
                            :, i * i_per_crude_bin : (i + 1) * i_per_crude_bin
                        ],
                        axis=1,
                    )
                    / crude_shell_volumes[i]
                )
                ...
                if redshift_bins[i] < redshifts[n_redshifts_detection]:
                    ...
                    binned_detection_rate[:, i] = np.sum(
                        detection_rate[
                            :, i * i_per_crude_bin : (i + 1) * i_per_crude_bin
                        ],
                        axis=1,
            ...
            detection_index = (
                z_index
                if z_index < n_redshifts_detection
                else n_redshifts_detection
            )

            print(
                "You will only save data up to redshift ",
                maxz,
                ", i.e. index",
                z_index,
            )
        ...
        DCO_to_rate_mask = (
            COMPAS.DCOmask
        )  # save this bool for easy conversion between BSE_Double_Compact_Objects, and CI weights
        rate_data_list = [
            DCO["SEED"][DCO_to_rate_mask],
            DCO_to_rate_mask,
            save_redshifts,
            save_merger_rate,
            merger_rate[:, 0],
        ...
        for i, data in enumerate(rate_data_list):
            ...
            dataNew = h_new[new_rate_group].create_dataset(
                rate_list_names[i], data=data
            )

    # Always close your files again ;)
    h_new.close()
    print(
        ("Done with append_rates :) your new files are here: {}".format(path))
    )
...
def delete_rates(
    ...
    with h5.File(path, "r+") as h_new:
        ...
        if new_rate_group not in h_new:
            ...
            print(
                "You want to remove this group, %s, from the hdf5 file, removing now.."
                % (new_rate_group)
            )
...
def plot_rates(
    ...
    axes[0, 0].set_ylabel(
        r"Formation rate $[\rm \frac{\mathrm{d}N}{\mathrm{d}Gpc^3 \mathrm{d}yr}]$",
        fontsize=fs,
    )
    ...
    axes[0, 1].set_ylabel(
        r"Merger rate $[\rm \frac{\mathrm{d}N}{\mathrm{d}Gpc^3 \mathrm{d}yr}]$",
        fontsize=fs,
    )

    axes[1, 0].plot(
        redshifts[: len(cumulative_detection_rate)],
        cumulative_detection_rate,
        lw=lw,
    )
    axes[1, 0].set_xlabel("Redshift", fontsize=fs)
    axes[1, 0].set_ylabel(
        r"Cumulative detection rate $[\rm \frac{\mathrm{d}N}{\mathrm{d}yr}]$",
        fontsize=fs,
    )

    axes[1, 1].hist(
        chirp_masses, weights=detection_rate_by_binary, bins=25, range=(0, 50)
    )
    axes[1, 1].set_xlabel(r"Chirp mass, $\mathcal{M}_c$", fontsize=fs)
    axes[1, 1].set_ylabel(
        r"Mass distrbution of detections $[\rm \frac{\mathrm{d}N}{\mathrm{d}\mathcal{M}_c \mathrm{d}yr}]$",
        fontsize=fs,
    )
    ...
    plt.savefig(
        save_dir
        + "Rate_Info"
        + "mu0%s_muz%s_alpha%s_sigma0%s_sigmaz%s"
        % (mu0, muz, alpha, sigma0, sigmaz)
        + ".png",
        bbox_inches="tight",
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--path",
        dest="path",
        help="Path to the COMPAS file that contains the output",
        type=str,
    ...
    parser.add_argument(
        "--maxz",
        dest="max_redshift",
        help="Maximum redshift to use in array",
        type=float,
        default=10,
    )
    parser.add_argument(
        "--zSF",
        dest="z_first_SF",
        help="redshift of first star formation",
        type=float,
        default=10,
    )
    parser.add_argument(
        "--maxzdet",
        dest="max_redshift_detection",
        help="Maximum redshift to calculate detection rates",
        type=float,
        default=1,
    )
    parser.add_argument(
        "--zstep",
        dest="redshift_step",
        help="size of step to take in redshift",
        type=float,
        default=0.001,
    )
    parser.add_argument(
        "--sens",
        dest="sensitivity",
        help="Which detector sensitivity to use: one of ['design', 'O1', 'O3']",
        type=str,
        default="O3",
    )
    parser.add_argument(
        "--snr",
        dest="snr_threshold",
        help="What SNR threshold required for a detection",
        type=float,
        default=8,
    )
    ...
    parser.add_argument(
        "--m1min",
        dest="m1_min",
        help="Minimum primary mass sampled by COMPAS",
        type=float,
        default=5.0,
    )
    parser.add_argument(
        "--m1max",
        dest="m1_max",
        help="Maximum primary mass sampled by COMPAS",
        type=float,
        default=150.0,
    )
    parser.add_argument(
        "--m2min",
        dest="m2_min",
        help="Minimum secondary mass sampled by COMPAS",
        type=float,
        default=0.1,
    )
    parser.add_argument(
        "--fbin",
        dest="fbin",
        help="Binary fraction used by COMPAS",
        type=float,
        default=0.7,
    )
    ...
    parser.add_argument(
        "--mu0",
        dest="mu0",
        help="mean metallicity at redshhift 0",
        type=float,
        default=0.035,
    )
    parser.add_argument(
        "--muz",
        dest="muz",
        help="redshift evolution of mean metallicity, dPdlogZ",
        type=float,
        default=-0.23,
    )
    parser.add_argument(
        "--sigma0",
        dest="sigma0",
        help="variance in metallicity density distribution, dPdlogZ",
        type=float,
        default=0.39,
    )
    parser.add_argument(
        "--sigmaz",
        dest="sigmaz",
        help="redshift evolution of variance, dPdlogZ",
        type=float,
        default=0.0,
    )
    parser.add_argument(
        "--alpha",
        dest="alpha",
        help="skewness of mtallicity density distribution, dPdlogZ",
        type=float,
        default=0.0,
    )
    parser.add_argument(
        "--aSF",
        dest="aSF",
        help="Parameter for shape of SFR(z)",
        type=float,
        default=0.01,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--dontAppend",
        dest="append_rates",
        help="Prevent the script from appending your rates to the hdf5 file.",
        action="store_false",
        default=True,
    )
    parser.add_argument(
...
def run_fast_cosmic_integration(args):
    #####################################
    ...
    if args.append_rates:
        n_redshifts_detection = int(
            args.max_redshift_detection / args.redshift_step
        )
        append_rates(
            args.path,
            detection_rate,
            ...
            COMPAS,
            n_redshifts_detection,
            maxz=args.max_redshift_detection,
            sensitivity=args.sensitivity,
            dco_type=args.dco_type,
            mu0=args.mu0,
            muz=args.muz,
    ...
    chirp_masses = (COMPAS.mass1 * COMPAS.mass2) ** (3.0 / 5.0) / (
        COMPAS.mass1 + COMPAS.mass2
    ) ** (1.0 / 5.0)
    plot_rates(
        args.path,
...
if __name__ == "__main__":
    args = parse_args()
    run_fast_cosmic_integration(args)
