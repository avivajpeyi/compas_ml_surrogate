...
def make_param_table(inference_result: bilby.result.Result, training_cache: LikelihoodCache = None) -> pd.DataFrame:
...
def make_inference_plots(
        mock_npz: str, inference_json: str, det_matrix_h5: str, data_cache: LikelihoodCache,
        model: Model):
    ...
    horizontal_concat(
        fnames, f"{outdir}/sampling_summary.png", rm_orig=False
    )


def plot_universes(inference_result: bilby.core.result.Result, det_matrix_h5: str, outdir: str, mock_npz: str):
...
def plot_sampling_corner(inference_result: bilby.core.result.Result, data_cache: LikelihoodCache, model: Model,
                         outdir: str):
    ...
    bins = {p: np.histogram(inference_result.posterior[p], bins=bins_size)[1] for p in plot_parameters}
    ...
    fig = add_legend_to_corner(fig, labels=['Inferred', 'Train', "True"], colors=['C0', 'C2', "tab:orange"])
...
def make_summary_page(outdir, table, cell_height=8):
    ...
    for i in range(0, len(table)):
        for col in range(0, len(table.columns)):
            ...
            pdf.cell(w=40, h=ch,
                     txt=table[col_name].iloc[i],
                     border=1, ln=col, align='C')
